...
import {appx} from './modules/app.js'
import { clearAllDrafts } from '@/utils/storage.js';

Vue.use(Vuex)

// 保存用户认证信息到 storage
function saveAuthToStorage({ user, token, role }) {
  try {
...
export default new Vuex.Store({
	...
	actions: {
	    ...
	    initUser({ commit }) {
	      try {
	        ...
	        if (u) {
	          ...
	          commit('SET_AUTH', { user, token: t, role: r })
	    ...
	    login({ commit }, options) {
	      ...
	      const token = options.token || '' //按照接口实际返回数据更改
        ...
        console.log('updateUserInfo', options)
