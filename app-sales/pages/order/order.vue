  ...
  <view class="container">
    <u-sticky offset-top="0" bgColor="">
      <view class="fliter-wrap">
        <u--input border="none" shape="circle" prefixIcon="search" prefixIconStyle="font-size: 44rpx;color: #909399"
        ...
        <view class="order-status-wrap">
          ...
          <view class="status" @click="statusShow = true">
            <text v-if="orderStatusName">{{orderStatusName}}</text>
            <text v-else style="color: #838383">未选择</text>
            <u-icon name="arrow-down" color="#999" size="28"></u-icon>
          </view>
          <u-button text="筛选" shape="circle" size="small"
            color="#2449FF; font-size: 26rpx; font-weight: bold; border-radius: 28rpx; width: auto;"
            @click="filterSubmit"></u-button>
        </view>
      </view>
      ...
      <u-list @scrolltolower="scrolltolower" :scroll-top="cardListScrollTop" ref="uListRef">
        <u-list-item v-for="(item, index) in cardList" :key="index">
          <view class="card-list-item" :style="index == 0 ? 'margin-top: 6rpx' : ''" @click="goDetail(item)">
            <view class="tag" :class="proStatus(item.statusTxt).className || ''">
              {{proStatus(item.statusTxt).label || ''}}
            </view>
    ...
    ></u-datetime-picker>
    <u-picker :show="statusShow" :columns="statusColumns" :visibleItemCount="7" keyName="label" @cancel="statusShow = false" @confirm="statusConfirm"></u-picker>
  </view>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      orderStatus: '0',
      orderStatusLabel: '未选择',
      statusShow: false,
      statusColumns: [
          [
            { id: 0, label: '未选择', className: '' },
  ...
  methods: {
    ...
    statusConfirm(e) {
      ...
      this.orderStatusLabel = e.value[0].label;
    ...
    filterSubmit() {
      ...
      console.log(this.keyword, this.date1Value, this.date2Value, this.orderStatus, this.orderStatusLabel)
      this.total = 0;
      this.loadmore();
    },
    tabClick(index) {
    ...
    async loadmore() {
      ...
      const res = await contractList({
        ...
        statusTxt: this.orderStatusLabel,
        orderStatus: this.orderStatus,
      });
      if (res.code == 200) {
        this.cardList = [];
        this.total = 0;
        console.log('this.orderStatusLabel', this.orderStatusLabel)
        const orderStatusLabel = this.orderStatusLabel;
        const timeout = setTimeout(() => {
          res.rows.forEach(item => {
            if (item.statusTxt != '未签约') {
              if (orderStatusLabel == '未选择') {
                this.cardList.push(item);
                this.total++;
              } else if (orderStatusLabel != '未选择' && item.statusTxt == orderStatusLabel) {
                this.cardList.push(item);
                this.total++;
              }
